...
if (isMobile.any()) {
	...
	let heroArrows = document.querySelectorAll('.hero-arrow');

	if (heroArrows.length > 0) {
		for (let index = 0; index < heroArrows.length; index++) {
			const heroArrow = heroArrows[index];
			heroArrow.addEventListener('click', function (e) {
				heroArrow.parentElement.classList.toggle('_active');
			});
		}
	}
}
...
if (navLinks.length > 0) {
	...
	function onMenuLinkClick(e) {
		...
		if (navLink.dataset.goto && document.querySelector(navLink.dataset.goto)) {
			...
			const gotoBlockValue = gotoBlock.getBoundingClientRect().top + window.pageYOffset - document.querySelector('.head__wrapper').offsetHeight;
...
// Подключение анимации при скроле
...
const filterMenu = document.querySelectorAll('.list-item__menu');

document.querySelector('.nav-menu__list').addEventListener('click', (event) => {
    ...
    filterMenu.forEach(elem => {
        ...
        if (!elem.classList.contains(filterClass) && filterClass !== 'menu') {
			elem.classList.add('hide');
        }
	});
});

const filterItems = document.querySelectorAll('.menu__list-item');
const actives = document.getElementsByClassName('active');
	  
		for (let i = 0; filterItems.length > i; i++) {
			filterItems[i].onclick = function (event) {
...
function mapAdd() {
	...
	tag.src = "https://maps.google.com/maps/api/js?key=AIzaSyA_CHqr37HW8hhC1CAWp7h6yba_xyJ0f8o&callback=mapInit";
...
function map(n) {
	...
	function init() {
		...
		var myMap = new ymaps.Map("map", {
			...
			controls: [],
			center: [49.58827, 34.55142],
			// Уровень масштабирования. Допустимые значения:
			// от 0 (весь мир) до 19.
			zoom: 10
		});

		let myPlacemark = new ymaps.Placemark([49.58827, 34.55142], {
		},{
			// Опции.
